"""
Frequency-weighted iterative local linearization for materialization optimization.

Same algorithm as iterative_local_linearization.py, but multiplies B[k] and U[(p,k)]
by each node's frequency cost multiplier before passing to the ILP solver.

Usage:
    python iterative_local_linearization_freq.py <base_filename> \
...
from utility.analyze import add_scaled_inplace, solve_slot_time_ilp, summarize_iteration, build_parent_child_maps
from generate_frequency_profiles import (
    propagate_to_cost_multipliers,
    FREQ_COST_MAP,
)
...
def parse_args():
    parser = argparse.ArgumentParser(
        description="Frequency-weighted iterative local linearization for materialization optimization."
    )
    parser.add_argument(
        "base_filename",
...
# Output path includes scenario name to avoid overwriting
NEW_MATERIALIZATION_OUTPUT_PATH = (
    f"useful_files/tuva_analysis/for_materialization_tuning/new_materializations/"
    f"{base_filename}_freq_{scenario_name}_new_materialization.json"
)
...
# ============= re-propagate frequencies using this manifest's DAG ===============
# Root/leaf assignments are fixed external facts; only interior propagation
# depends on DAG topology, so we re-propagate to cover new refactored nodes.
...
print(f"Nodes with frequency weight: {len(node_frequencies)}")
...
while True:
    ...
    # ===== Apply frequency weighting to B and U =====
    # B[k] and U[(p,k)] represent costs incurred each time node k runs.
    # Multiply by freq[k] so the ILP minimizes total frequency-weighted cost.
